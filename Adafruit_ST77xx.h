...
public:
    ...
    inline void enableDisplay(boolean enable) { sendCommand(enable ? ST77XX_DISPON : ST77XX_DISPOFF); }
    inline void enableTearing(boolean enable) { sendCommand(enable ? ST77XX_TEON : ST77XX_TEOFF); }
    inline void enableSleep(boolean enable) { sendCommand(enable ? ST77XX_SLPIN : ST77XX_SLPOUT); }
    inline void setColRowStart(int8_t col, int8_t row) { _colstart = col; _rowstart = row; }
};
...
static const uint8_t PROGMEM st7796s_init[] = 
    ...
    , ST77XX_MADCTL, 1, 0x48  // Memory Access
    , ST77XX_COLMOD, 1, 0x55 // Color Mode - 16 bit
    ...
    , 0xF0, 1, 0x3C
    , ST77XX_SLPOUT, ST_CMD_DELAY, 150 // Exit sleep
    , ST77XX_DISPON, ST_CMD_DELAY, 150 // Display on
    };
